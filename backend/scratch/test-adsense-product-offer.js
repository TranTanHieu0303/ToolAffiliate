...
async function main() {
  ...
  console.log('Clicking Product Offer...');
  await page.evaluate(() => {
    const pMenu = document.querySelector('li[title="adsense.menu.partner-offer"]');
    if (pMenu) {
      pMenu.click();
    } else {
      const els = Array.from(document.querySelectorAll('.next-menu-item-text'));
      const pEl = els.find(el => el.textContent && el.textContent.trim() === 'Product Offer');
      if (pEl) pEl.click();
    }
  });

  console.log('Waiting for Product Offer page to load...');
  ...
  // Take screenshot
  const screenshotPath = path.join(__dirname, 'adsense_product_offer_page.png');
